...
## ⚠️ Activer l'onglet Administration (à faire UNE FOIS au déploiement)

L'onglet **Administration** est **caché par défaut** et ne s'affiche que pour les comptes promus en super-administrateur. Si vous ne le voyez pas, exécutez le script [`bootstrap-admin.sql`](./bootstrap-admin.sql) :

1. Ouvrez le fichier `bootstrap-admin.sql`.
2. À l'intérieur, remplacez `'votre.email@example.com'` par votre adresse e-mail de connexion au CRM.
3. Collez la requête dans Supabase → SQL Editor → New query → **Run**.
4. **Déconnectez-vous puis reconnectez-vous** au CRM : l'onglet "Administration" apparaît dans la barre latérale.

Le script vérifie que l'utilisateur existe, crée son profil si nécessaire, et est idempotent (peut être rejoué sans risque).

## Mise à jour du tour actuel

### Corrections

- **Bug du select Contact dans la modale Contrat** : `populateContactSelects()` est désormais rappelé à chaque ouverture de la modale Contrat (et de la modale Tâche), pour garantir qu'un contact fraîchement ajouté apparaît immédiatement dans la liste déroulante.

### Nouveautés

- **Suppression définitive d'un utilisateur** (en plus de la révocation déjà existante). Schéma `v10` : nouvelle fonction RPC `admin_delete_user`, accessible uniquement aux super-administrateurs, avec :
  - **double confirmation** (boîte de dialogue + saisie du prénom/email pour valider l'action irréversible),
  - **protection** contre l'auto-suppression de son propre compte,
  - **conservation** des contacts/contrats/tâches créés par l'utilisateur supprimé (le champ "Ajouté par" devient "—"),
  - **cascade SET NULL** sur les contraintes FK pour éviter les erreurs.

- **Onglet Objectifs enrichi pour les super-administrateurs** : en plus de ses propres 3 jauges (Entrées en contact, CA généré, Commissions), l'admin voit en bas une section **"Performance de l'équipe"** avec une ligne par utilisateur (photo + prénom + 3 mini-jauges colorées). Les utilisateurs non-admin ne voient que leurs propres jauges.

### Étapes à appliquer

1. Exécutez [`bootstrap-admin.sql`](./bootstrap-admin.sql) si l'onglet Administration n'apparaît pas chez vous.
2. Exécutez [`supabase-schema-v10.sql`](./supabase-schema-v10.sql) (suppression définitive des utilisateurs).
3. Poussez sur GitHub : `index.html`, `assets/app.js`.

## Mise à jour majeure (tour actuel)

### Tunnel d'envoi désactivé

L'envoi automatique sur kDrive et par e-mail est **supprimé**. Le bouton « Bon de commande PDF » de la modale Contrat produit maintenant **un seul PDF combiné** contenant :
- les pages du Bon de commande (formule, parties, récap financier, signatures « Bon pour accord »),
- les pages des **Conditions Générales de Vente** S@FE (réf. `SAFE-CGV-2026-001`).

L'utilisateur le télécharge puis le transmet manuellement au client par le canal de son choix.

> Les fichiers `supabase/functions/send-contract/`, les secrets associés (`KDRIVE_*`, `SMTP_*`) et la modale signature canvas n'ont plus d'utilité. Vous pouvez :
> - **supprimer la fonction edge** depuis le dashboard Supabase (Edge Functions → send-contract → Delete),
> - **retirer les secrets** (Project Settings → Edge Functions → Secrets),
> - **révoquer définitivement** le token kDrive et le mot de passe d'app iCloud (geste de sécurité recommandé puisqu'ils ont transité par notre conversation).

### Propriété stricte des fiches clients

À exécuter dans Supabase : [`supabase-schema-v11.sql`](./supabase-schema-v11.sql).

- **Une fiche contact est désormais la propriété de son créateur** (champ `created_by`). Seul le propriétaire ou un super-administrateur peut la modifier ou la supprimer. Les autres utilisateurs continuent à la voir en lecture.
- Dans la liste des contacts, les fiches d'autres utilisateurs portent un petit cadenas 🔒. Quand on les ouvre, un bandeau « Cette fiche appartient à X » s'affiche et les boutons Enregistrer/Supprimer disparaissent.
- Même règle pour les **contrats** : un contrat est lié à un contact, et seul le propriétaire du contrat (ou un admin) peut le modifier.

### Statut « Terminé » réservé aux super-administrateurs

- Dans la modale Contrat, les options « Terminé » et « Résilié » sont **grisées** pour les commerciaux. Ils peuvent faire évoluer leur contrat jusqu'à « Signé » ou « En cours », mais c'est l'admin qui valide la clôture définitive.
- Côté serveur, un **trigger Postgres** rejette toute tentative de bascule vers ces statuts par un non-admin (sécurité défense en profondeur, même si quelqu'un contourne l'UI).

### Transfert d'un client

- Nouveau bouton **🔄 Transférer client** dans la fiche contact (visible pour le propriétaire ou un admin).
- Modale dédiée : sélection de l'utilisateur destinataire dans la liste, double confirmation.
- Le transfert réassigne automatiquement **tous les contrats et tâches** liés au client.
- Implémenté via la fonction RPC `transfer_contact(p_contact_id, p_target_user_id)` qui vérifie côté Supabase que l'appelant est bien propriétaire ou admin.

### Étapes à appliquer

1. Exécutez [`supabase-schema-v11.sql`](./supabase-schema-v11.sql) dans Supabase.
2. Poussez sur GitHub : `index.html`, `assets/app.js`, `assets/contract-pdf.js`. Vous pouvez supprimer `assets/contract-sign.js` (n'est plus inclus).
